...
# --------------------------- private: initialize and load -------------------------- #
    def __init__(self, distro_path):                
        ...
        self._publicids      = {}     #key = item_id; value = item_publicid
        self._redactions     = {}     #key = original string; value = generated string
        self._folderitems    = {}     #key = folder_id; value = item_publicid
        self._itemids        = {}     #key = item_publicid; value = item_id
        self._load()
    ...
    def _load_publicids(self, db):
        ...
        for r in rows:
            item_id  = r[0]
            publicid = r[1]
            self._publicids[item_id] = publicid
    ...
    def _load_pstfolders(self, db):
        ...
        for r in rows:
            ...
            if item_id in self._items:
                self._load_dictionaries(folder_id, item_id, public_id)
    ...
    def _load_dictionaries(self, folder_id, item_id, public_id):
        ...
        if public_id not in d2:
            d2[public_id] = []
        d2[public_id].append(item_id)
...
# --------------------------- private: main functionality -------------------------- #
    def _make_dirs(self, distro_path):
        ...
        self.path_meta = os.path.join(distro_path, 'meta')
    ...
    def _write_custodian_xml_file(self, src_id, src_id_str, src_name, src_size, itemcount):
            ...
            self._build_custodian_tag(sb, src_id, src_size, src_name)
            sb.append('\n</custodian>')
            sb.append('\n\n')
    ...
    def _build_custodian_tag(self, sb, src_id, src_size, src_name):
        ...
        root_id = None
        rows = self._source_folders[src_id]
        for r in rows:
            id        = r[0]
            parent_id = r[1]
            name      = r[2]
        ...
        self._build_pst_node(sb, src_id, root_id, children, names, 1)
    ...
    def _write_metafile(self, item):
        sb = []
        type = get_type_by_publicid(item.public_id())
        sb.append('<item id="{0}" custodian_id="{1}" type="{2}" '.format(item.public_id(), item.source_id(), type))
        ...
        self._write_item_file(item, sb, 'xml', self.path_meta)
